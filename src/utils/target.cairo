...
func encode_target{range_check_ptr}(target: Uint256) -> (bits: felt) {
...
namespace internal {
    ...
    func encode_target{range_check_ptr}(target: Uint256, negative: felt) -> (bits: felt) {
        alloc_locals;
        local bytes: felt*;
        local size;
    ...
    func get_bytes_128{range_check_ptr}(x) -> (bytes_len: felt, bytes: felt*) {
        alloc_locals;
    ...
    func get_truncated_target(size, bytes_len, bytes: felt*, acc: felt) -> (res: felt) {
        if (size == 0) {
            return (acc,);
        }
        if (bytes_len == 0) {
