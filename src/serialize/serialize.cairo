...
func _write_n_bytes_into_felt{writer: Writer, range_check_ptr}(
    input: felt, value, base, loop_counter
) {
    alloc_locals;
    if (loop_counter == 0) {
        return ();
    }
    let (local first_byte, local other_bytes) = unsigned_div_rem(input, BYTE ** 3 / base);
    write_uint8(first_byte);
    _write_n_bytes_into_felt(other_bytes, first_byte * base + value, base * BYTE, loop_counter - 1);
    return ();
}

func _write_into_uint32_array{writer: Writer, range_check_ptr}(input: felt*, loop_counter) {
    if (loop_counter == 0) {
        return ();
    }
    _write_n_bytes_into_felt([input], 0, 1, UINT32_SIZE);
    _write_into_uint32_array(input + 1, loop_counter - 1);
    return ();
}

func write_bytes{writer: Writer, range_check_ptr}(input: felt*, length: felt) {
    alloc_locals;
    let (len_div_4, len_mod_4) = unsigned_div_rem(length, UINT32_SIZE);
    // Write as many 4-byte chunks as possible
    _write_into_uint32_array(input, len_div_4);
    // Write up to three more bytes
    _write_n_bytes_into_felt([input + len_div_4], 0, 1, len_mod_4);
    return ();
}
